...
ag_mean_bl = np.array([0.3283, 0.5420, 0.7801, 0.8188, 0.8785, 0.9092, 0.9112])
ag_std_bl  = np.array([0.1697, 0.1195, 0.0453, 0.0269, 0.0159, 0.0081, 0.0037])
...
# --- CBAM 数据: 单次 Run 结果 ---
cbam_bl = np.array([0.2365, 0.6922, 0.7918, 0.8295, 0.8813, 0.9073, 0.9092])
...
plot_with_range(xtr, ag_mean_bl, ag_std_bl, cbam_bl, 'Linear Probing', "#cb20d4")

# 3. 图表修饰
plt.xscale('log')
plt.xticks(xtr, xtr)
plt.ylim(0.2, 1.0) # 根据数据范围调整起始点
