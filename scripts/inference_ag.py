...
def run_multi_sigma_test(model, image, model_name, k_val, sigmas=[0, 1, 2, 4]):
    ...
    for i, s in enumerate(sigmas):
        ...
        with torch.no_grad():
            output = model.model(input_img, store_att_maps=True)
...
if __name__ == '__main__':
    ...
    for k in Xtr:
        ...
        print("Proposed")
        cl_model = CL_model.CL_Model(cfg)
        cl_model.load_backbone_model(cfg.contrastive_pretraining.save_path_backbone.split(".")[0]+f"_al_{k}.pth")
        cl_model.load_outconv_model(cfg.contrastive_pretraining.save_path_outconv_layer.split(".")[0]+f"_al_{k}.pth")
        # test_losses, test_losses_detailed = cl_model.run_test_volume(test_volume_loader)
        # print(f"Avg. Dice (ACDC): {1 - test_losses}")
        run_multi_sigma_test(cl_model, fixed_image, "Ablation", k)
